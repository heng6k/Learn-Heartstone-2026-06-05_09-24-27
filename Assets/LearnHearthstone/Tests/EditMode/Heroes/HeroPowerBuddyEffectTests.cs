...
namespace LearnHearthstone.Tests.EditMode
{
    public sealed class HeroPowerBuddyEffectTests
    {
        ...
        public void SnakeEyes_LuckyRollGainsRolledGoldAndCooldownsByRoll()
        {
            var service = CreateHeroService(
                "BG28_HERO_400",
                new MatchSetupOptions { EnableTrinkets = false, EnableTimewarpedTavern = false });
            var tavern = service.State.Player.Tavern;
            tavern.Gold = 10;
            tavern.MaxGold = 20;
        ...
        public void JandiceMutanusAndXyrella_CoreTavernActionsResolve()
        {
            ...
            StatMath.ApplyEnchantment(xyrella.State.Player.Tavern.Shop[0], new Enchantment
            {
                Id = "Xyrella Blood Gem",
                SourceId = "Xyrella Blood Gem",
            ...
            Assert.AreEqual(1, handCard.Enchantments.Count(StatMath.IsBloodGemEnchantment));
            Assert.IsTrue(handCard.Enchantments.Any(enchantment => enchantment.Kind == EnchantmentKind.SetStats));
        }
