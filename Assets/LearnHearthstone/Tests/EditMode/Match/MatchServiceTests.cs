...
namespace LearnHearthstone.Tests.EditMode
{
    public sealed class MatchServiceTests
    {
        ...
        public void TimewarpedTavern_ExitOfferClosesVisitWithoutAddingHandCard()
        {
            ...
            Assert.AreEqual(0, timewarp.Chronum);
        ...
        [Test]
        public void TimewarpedTavern_EnabledSchedulesVisitWithoutAdvancedMechanicMode()
        {
            var service = MatchService.CreateWithDefaultCatalog(
                12345,
                null,
            ...
            Assert.AreEqual(TimewarpKind.Minor, service.State.Player.Tavern.Timewarp.PendingKind);
        }
        ...
        public void TimewarpedTavern_RoundSixOpensMinorAndPurchasesWithChronum()
        {
            ...
            AssertOfficialTimewarpedOfferComposition(service);
            Assert.IsTrue(timewarp.Offers.Any(candidate => candidate.CardId.StartsWith("BG34_HeroPowerSpell_", System.StringComparison.OrdinalIgnoreCase)));
        ...
        [Test]
        public void TimewarpedTavern_RoundNineOpensMajorWithFreshChronum()
        {
            var service = CreateTimewarpOnlyService(12345);
            AdvanceToRound(service, 6);
        ...
        [Test]
        public void TimewarpedTavern_MurozondAddsTurnEightVisitWithoutReplacingStandardVisits()
        {
            var service = MatchService.CreateWithDefaultCatalog(
                12345,
                null,
        ...
        [Test]
        public void TimewarpedTavern_HandFullPurchaseTemporarilyExpandsHand()
        {
            var service = CreateTimewarpOnlyService(12345);
            AdvanceToRound(service, 6);
            var tavern = service.State.Player.Tavern;
            ...
            {
                var suffix = tavern.Hand.Count.ToString();
                tavern.Hand.Add(TestBoardMinion("full-hand-" + suffix, "Full Hand", "FULL_HAND_" + suffix, 1, 1, Tribe.Beast, 1));
            }
            ...
            Assert.AreEqual(chronumBefore - 1, timewarp.Chronum);
            Assert.IsTrue(offer.Purchased);
            Assert.AreEqual(11, tavern.Hand.Count);
            Assert.IsTrue(timewarp.HasTemporaryHandExpansion);

            service.Apply(new GameCommand(GameCommandType.ExitTimewarpedTavern));
            service.Apply(new GameCommand(GameCommandType.PlayMinion, 0));
            Assert.AreEqual(10, tavern.Hand.Count);
            Assert.IsFalse(timewarp.HasTemporaryHandExpansion);
        }

        [Test]
        public void TimewarpedTavern_OpenVisitBlocksNormalTavernCommandsAtServiceBoundary()
        {
            var service = CreateTimewarpOnlyService(12345);
            AdvanceToRound(service, 6);
            var tavern = service.State.Player.Tavern;
            var before = TavernShopStateFingerprint(tavern);

            var exception = Assert.Throws<System.InvalidOperationException>(() =>
                service.Apply(new GameCommand(GameCommandType.RerollShop)));

            Assert.AreEqual("时空酒馆开启期间只能购买时空卡牌或退出。", exception.Message);
            Assert.AreEqual(before, TavernShopStateFingerprint(tavern));
            Assert.IsFalse(service.CanApply(GameCommandType.RerollShop));
            Assert.IsTrue(service.CanApply(GameCommandType.BuyTimewarpedTavernCard));
            Assert.IsTrue(service.CanApply(GameCommandType.ExitTimewarpedTavern));
        }

        [Test]
        public void TimewarpedTavern_MinorVisitOmitsSecondHeroPowerWhenTwoAreOwned()
        {
            var service = CreateTimewarpOnlyService(12345);
            service.State.Player.ExtraHeroPowerCardIds.Add("TB_BaconShop_HP_010");

            AdvanceToRound(service, 6);

            AssertOfficialTimewarpedOfferComposition(service);
            Assert.IsFalse(service.State.Player.Tavern.Timewarp.Offers.Any(candidate =>
                candidate.CardId.StartsWith("BG34_HeroPowerSpell_", System.StringComparison.OrdinalIgnoreCase)));
        }

        [Test]
        public void TimewarpedTavern_FiveOffersIgnoreNormalShopCapacity()
        {
            foreach (var shopCount in new[] { 3, 7 })
            {
                ...
                {
                    tavern.Shop.RemoveAt(tavern.Shop.Count - 1);
                }

                while (tavern.Shop.Count < shopCount)
                {
                    var suffix = tavern.Shop.Count.ToString();
                    tavern.Shop.Add(TestBoardMinion("shop-" + suffix, "Shop", "SHOP_" + suffix, 1, 1, Tribe.Beast, 1));
                }

                Assert.AreEqual(shopCount, tavern.Shop.Count);
                AssertOfficialTimewarpedOfferComposition(service);
                CollectionAssert.AreEqual(timewarpedCards, tavern.Timewarp.Offers.Select(offer => offer.CardId).ToList());
                CollectionAssert.AreEqual(new[] { 0, 1, 2, 3, 4 }, tavern.Timewarp.Offers.Select(offer => offer.SlotIndex).ToArray());
            }
        }

        [Test]
        public void TimewarpedTavern_InvalidExplicitPoolDoesNotLeavePartialOpenState()
        {
            var baseline = CreateTimewarpOnlyService(12345);
            var onlyMinion = baseline.GetTimewarpedCandidateDefinitions(TimewarpKind.Minor)
                .First(card => card.CardKind == CardKind.Minion);
            var service = MatchService.CreateWithDefaultCatalog(
                12345,
                null,
                new MatchSetupOptions
                {
                    ...
                    EnableTrinkets = false,
                    UseExplicitTimewarpedPool = true,
                    EnabledTimewarpedCardIds = new List<string> { onlyMinion.CardId }
                });
        ...
        public void TimewarpedTavern_SmartOffersPrioritizeBoardAndHandDirections()
        {
            ...
            Assert.AreEqual(5, offers.Select(card => card.CardId).Distinct().Count());
            Assert.AreEqual(3, offers.Count(card => card.CardKind == CardKind.Minion));
            Assert.AreEqual(2, offers.Count(card => card.CardKind != CardKind.Minion));
            Assert.AreEqual(3, offers.Count(card => card.Cost == 1));
            Assert.AreEqual(2, offers.Count(card => card.Cost >= 2));
        ...
        [Test]
        public void Apply_ConsecutiveFreeRefreshesUseDifferentRefreshSequences()
        {
            var service = MatchService.CreateWithDefaultCatalog(12345);
            var tavern = service.State.Player.Tavern;
            tavern.FreeRefreshes = 2;

            service.Apply(new GameCommand(GameCommandType.RerollShop));
            var firstCards = tavern.Shop.Where(card => card != null).Select(card => card.CardId).ToList();

            service.Apply(new GameCommand(GameCommandType.RerollShop));
            var secondCards = tavern.Shop.Where(card => card != null).Select(card => card.CardId).ToList();

            CollectionAssert.AreNotEqual(firstCards, secondCards);
        }
        ...
        public void Apply_PlayingTavernSpellDoesNotPutItOnBoard()
        {
            ...
            var targetIndex = service.State.Player.Tavern.Shop.FindIndex(card => card != null && card.CardKind == CardKind.Minion);
            service.Apply(new GameCommand(GameCommandType.PlayMinion, service.State.Player.Tavern.Hand.Count - 1, targetIndex, TargetZone.TavernShop, -1, TargetZone.Unspecified));
        ...
        public void Apply_TierTwoBattlecriesAndGeneratedSpellsResolve()
        {
            ...
            service.Apply(new GameCommand(GameCommandType.PlayMinion, service.State.Player.Tavern.Hand.FindIndex(card => card.CardId == "SLIMY_SHIELD"), 0, TargetZone.FriendlyBoard, -1, TargetZone.Unspecified));
        ...
        public void Apply_TierTwoCombatDeathrattleRewardsApplyAfterCombatTest()
        {
            ...
            bully.Apply(new GameCommand(GameCommandType.PlayMinion, specialGemIndex, 0, TargetZone.FriendlyBoard, -1, TargetZone.Unspecified));
        ...
        public void Apply_TierTwoSpellcraftSpellsGenerateAndResolveAsNormalSpells()
        {
            ...
            reef.Apply(new GameCommand(GameCommandType.PlayMinion, reefSpellIndex, 0, TargetZone.FriendlyBoard, -1, TargetZone.Unspecified));
            ...
            lava.Apply(new GameCommand(GameCommandType.PlayMinion, 0, 0, TargetZone.FriendlyBoard, -1, TargetZone.Unspecified));
            ...
            surf.Apply(new GameCommand(GameCommandType.PlayMinion, surfSpellIndex, 0, TargetZone.FriendlyBoard, -1, TargetZone.Unspecified));
        ...
        [Test]
        public void Spellcraft_PlayImmediatelyUsesTheFreedHandSlotAndGoldenPayload()
        {
            var service = MatchService.CreateWithDefaultCatalog(12345);
            var tavern = service.State.Player.Tavern;
            service.State.Player.Board.Clear();
            tavern.Hand.Clear();
            service.Apply(new GameCommand(GameCommandType.AddCardToHand, "BG27_004", CardKind.Minion));
            tavern.Hand[0].Golden = true;
            while (tavern.Hand.Count < 10)
            {
                var index = tavern.Hand.Count;
                tavern.Hand.Add(TestBoardMinion("spellcraft-full-" + index, "Filler", "FILLER_" + index, 1, 1, Tribe.Beast, 1));
            }
        ...
        [Test]
        public void Spellcraft_TurnStartUsesBoardOrderAndDoesNotQueueMissedCards()
        {
            var service = MatchService.CreateWithDefaultCatalog(12345);
            var tavern = service.State.Player.Tavern;
            service.State.Player.Board.Clear();
            tavern.Hand.Clear();
            service.State.Player.Board.Add(TestBoardMinion("surf-source", "Surf n' Surf", "BG27_004", 1, 1, Tribe.Naga, 1));
            service.State.Player.Board.Add(TestBoardMinion("reef-source", "Reef Riffer", "BG26_501", 1, 1, Tribe.Naga, 2));
            for (var index = 0; index < 9; index += 1)
            {
                tavern.Hand.Add(TestBoardMinion("ordered-hand-" + index, "Filler", "ORDERED_" + index, 1, 1, Tribe.Beast, 1));
            }

            service.Apply(new GameCommand(GameCommandType.DebugSkipToNextTurn));

            Assert.AreEqual(10, tavern.Hand.Count);
            Assert.AreEqual(1, tavern.Hand.Count(card => card.CardId == "SURF_N_SURF_SPELL"));
            Assert.IsFalse(tavern.Hand.Any(card => card.CardId == "REEF_RIFFER_SPELL"));

            tavern.Hand.RemoveAt(0);
            Assert.IsFalse(tavern.Hand.Any(card => card.CardId == "REEF_RIFFER_SPELL"));
        }

        [Test]
        public void Spellcraft_FullHandAtTurnStartDoesNotGenerate()
        {
            var service = MatchService.CreateWithDefaultCatalog(12345);
            var tavern = service.State.Player.Tavern;
            service.State.Player.Board.Clear();
            tavern.Hand.Clear();
            service.State.Player.Board.Add(TestBoardMinion("surf-full-source", "Surf n' Surf", "BG27_004", 1, 1, Tribe.Naga, 1));
            for (var index = 0; index < 10; index += 1)
            {
                tavern.Hand.Add(TestBoardMinion("full-turn-hand-" + index, "Filler", "FULL_TURN_" + index, 1, 1, Tribe.Beast, 1));
            }

            service.Apply(new GameCommand(GameCommandType.DebugSkipToNextTurn));

            Assert.AreEqual(10, tavern.Hand.Count);
            Assert.IsFalse(tavern.Hand.Any(card => card.CardId == "SURF_N_SURF_SPELL"));
        }

        [Test]
        public void Spellcraft_ExpiresBeforeOtherTurnEndEffectsUseHandSpace()
        {
            var service = MatchService.CreateWithDefaultCatalog(12345);
            var tavern = service.State.Player.Tavern;
            service.State.Player.Board.Clear();
            tavern.Hand.Clear();
            service.State.Player.Board.Add(TestBoardMinion("looter-source", "Lost City Looter", "BG33_820", 3, 3, Tribe.Pirate, 3));
            service.Apply(new GameCommand(GameCommandType.AddCardToHand, "SURF_N_SURF_SPELL", CardKind.Spell));
            for (var index = 0; index < 9; index += 1)
            {
                tavern.Hand.Add(TestBoardMinion("turn-end-hand-" + index, "Filler", "TURN_END_" + index, 1, 1, Tribe.Beast, 1));
            }

            service.Apply(new GameCommand(GameCommandType.DebugSkipToNextTurn));

            Assert.AreEqual(10, tavern.Hand.Count);
            Assert.IsFalse(tavern.Hand.Any(card => card.Tags.Contains("temporary_spellcraft_card")));
            Assert.AreEqual(1, tavern.Hand.Count(card => card.CardKind == CardKind.TavernSpell));
        }
        ...
        private static void AssertOfficialTimewarpedOfferComposition(MatchService service)
        {
            var offers = CurrentTimewarpedOfferDefinitions(service);
            Assert.AreEqual(5, offers.Count);
            Assert.AreEqual(5, offers.Select(card => card.CardId).Distinct().Count());
