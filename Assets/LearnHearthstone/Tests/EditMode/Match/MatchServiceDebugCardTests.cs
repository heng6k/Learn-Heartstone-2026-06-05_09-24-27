...
namespace LearnHearthstone.Tests.EditMode
{
    public sealed class MatchServiceDebugCardTests
    {
        ...
        [Test]
        public void Apply_DebugCommandsCreateEnabledTimewarpedMinions()
        {
            var service = MatchService.CreateWithDefaultCatalog(
                ...
                new MatchSetupOptions
                {
                    ...
                    EnabledTimewarpedCardIds = new System.Collections.Generic.List<string> { "BG34_Giant_591" }
                });
            service.State.Player.Tavern.Hand.Clear();

            service.Apply(new GameCommand(GameCommandType.AddCardToHand, "BG34_Giant_591", CardKind.Minion));
            service.Apply(new GameCommand(GameCommandType.AddOpponentMinion, "BG34_Giant_591", true));

            var handCard = service.State.Player.Tavern.Hand.Single();
            Assert.AreEqual("BG34_Giant_591", handCard.CardId);
            Assert.AreEqual(BoardSide.Player, handCard.Owner);
            Assert.AreEqual(PoolSource.Timewarped, handCard.PoolSource);
            var opponent = service.State.Opponent.Board.Single();
            Assert.AreEqual("BG34_Giant_591", opponent.CardId);
            Assert.AreEqual(BoardSide.Opponent, opponent.Owner);
            Assert.AreEqual(PoolSource.Timewarped, opponent.PoolSource);
            Assert.IsTrue(opponent.Golden);
        }
