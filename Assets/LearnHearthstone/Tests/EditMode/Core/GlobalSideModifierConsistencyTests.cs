...
namespace LearnHearthstone.Tests.EditMode
{
    ...
    {
        [Test]
        public void NewMatch_BeetleQualityStartsAtTwoTwoForBothSides()
        {
            var service = MatchService.CreateWithDefaultCatalog(12345, new InMemoryTestScenarioRepository());

            Assert.AreEqual(2, service.State.Player.Tavern.BeetleAttackBonus);
            Assert.AreEqual(2, service.State.Player.Tavern.BeetleHealthBonus);
            Assert.AreEqual(2, service.State.Player.CombatModifiers.BeetleAttackBonus);
            Assert.AreEqual(2, service.State.Player.CombatModifiers.BeetleHealthBonus);
            Assert.AreEqual(2, service.State.Opponent.CombatModifiers.BeetleAttackBonus);
            Assert.AreEqual(2, service.State.Opponent.CombatModifiers.BeetleHealthBonus);
        }

        [Test]
        public void DirectEditing_RoundTripsEveryGlobalModifierForBothSides()
        {
            var service = MatchService.CreateWithDefaultCatalog(12345, new InMemoryTestScenarioRepository());

            foreach (SideCombatModifierKind kind in Enum.GetValues(typeof(SideCombatModifierKind)))
            {
                service.Apply(new GameCommand(GameCommandType.SetSideCombatModifier, BoardSide.Player, kind, 7));
                service.Apply(new GameCommand(GameCommandType.SetSideCombatModifier, BoardSide.Opponent, kind, 9));

                Assert.AreEqual(7, SideModifierService.GetValue(service.State.Player.CombatModifiers, kind), "Player " + kind);
                Assert.AreEqual(9, SideModifierService.GetValue(service.State.Opponent.CombatModifiers, kind), "Opponent " + kind);
            }

            var playerMirror = new SideCombatModifierState();
            SideModifierService.CopyFromTavern(service.State.Player.Tavern, playerMirror);
            foreach (SideCombatModifierKind kind in Enum.GetValues(typeof(SideCombatModifierKind)))
            {
                Assert.AreEqual(7, SideModifierService.GetValue(playerMirror, kind), "Player Tavern mirror " + kind);
            }

            var opponentPreview = service.GetOpponentCombatTavernStatePreview();
            var opponentMirror = new SideCombatModifierState();
            SideModifierService.CopyFromTavern(opponentPreview, opponentMirror);
            foreach (SideCombatModifierKind kind in Enum.GetValues(typeof(SideCombatModifierKind)))
            {
                Assert.AreEqual(9, SideModifierService.GetValue(opponentMirror, kind), "Opponent combat preview " + kind);
            }
        }

        [Test]
        public void DirectEditing_ClampsBeetleQualityToTwoWithoutChangingOtherMinimums()
        {
            var service = MatchService.CreateWithDefaultCatalog(12345, new InMemoryTestScenarioRepository());

            service.Apply(new GameCommand(GameCommandType.SetSideCombatModifier, BoardSide.Player, SideCombatModifierKind.BeetleAttackBonus, -5));
            service.Apply(new GameCommand(GameCommandType.SetSideCombatModifier, BoardSide.Opponent, SideCombatModifierKind.BeetleHealthBonus, 0));
            service.Apply(new GameCommand(GameCommandType.SetSideCombatModifier, BoardSide.Player, SideCombatModifierKind.SpellPower, -5));

            Assert.AreEqual(2, service.State.Player.Tavern.BeetleAttackBonus);
            Assert.AreEqual(2, service.State.Opponent.CombatModifiers.BeetleHealthBonus);
            Assert.AreEqual(0, service.State.Player.Tavern.SpellPower);
        }

        [Test]
        public void Combat_BothSidesSummonBeetlesUsingTheirOwnEditedQuality()
        {
            var service = MatchService.CreateWithDefaultCatalog(12345, new InMemoryTestScenarioRepository());
            ...
            service.Apply(new GameCommand(GameCommandType.SetSideCombatModifier, BoardSide.Player, SideCombatModifierKind.BeetleAttackBonus, 7));
            service.Apply(new GameCommand(GameCommandType.SetSideCombatModifier, BoardSide.Player, SideCombatModifierKind.BeetleHealthBonus, 8));
            service.Apply(new GameCommand(GameCommandType.SetSideCombatModifier, BoardSide.Opponent, SideCombatModifierKind.BeetleAttackBonus, 9));
            service.Apply(new GameCommand(GameCommandType.SetSideCombatModifier, BoardSide.Opponent, SideCombatModifierKind.BeetleHealthBonus, 10));

            service.Apply(new GameCommand(GameCommandType.RunCombatTest, new CombatTestOptions { Seed = 441, SafetyLimit = 1 }));

            var playerBeetle = service.State.LastResult.FinalPlayerBoard.Single(card => card.Name == "Beetle");
            var opponentBeetles = service.State.LastResult.FinalOpponentBoard.Where(card => card.Name == "Beetle").ToList();
            Assert.AreEqual(7, playerBeetle.Attack);
            Assert.AreEqual(8, playerBeetle.MaxHealth);
            Assert.AreEqual(2, opponentBeetles.Count);
            Assert.IsTrue(opponentBeetles.All(card => card.Attack == 9 && card.MaxHealth == 10));
        }
