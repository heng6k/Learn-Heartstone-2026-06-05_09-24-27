...
namespace LearnHearthstone.Tests.EditMode
{
    public sealed class AnomalySystemTests
    {
        ...
        public void AnomalyCatalog_LoadsCurrentHsReplaySnapshot()
        {
            ...
            Assert.AreEqual(77, catalog.All.Count(anomaly => anomaly.ImplementationStatus == AnomalyImplementationStatus.Unsupported));
            Assert.AreEqual(77, catalog.All.Count(anomaly =>
        ...
        public void AnomalySetup_AllKnownKeepsUnsupportedHistoricalEntriesOutOfDefaultOffers()
        {
            ...
            Assert.AreEqual(34, offerable.Count);
            Assert.AreEqual(77, candidates.Count(anomaly => anomaly.ImplementationStatus == AnomalyImplementationStatus.Unsupported));
            ...
            CollectionAssert.IsSubsetOf(
                ...
                offerable.Select(anomaly => anomaly.CardId).ToList());
        ...
        [Test]
        public void TimewarpAnomalyCatalog_AllVisitModifiersAreImplemented()
        {
            var catalog = AnomalyCatalogLoader.LoadFromResources();
            ...
            {
                var anomaly = catalog.GetByCardId(cardId);
                Assert.AreEqual(AnomalyEffectFamily.Timewarp, anomaly.EffectFamily, cardId);
                Assert.AreEqual(AnomalyImplementationStatus.Implemented, anomaly.ImplementationStatus, cardId);
                CollectionAssert.Contains(anomaly.AvailabilityReasons, AnomalyAvailabilityReason.RequiresTimewarpPool);
            }
        }

        [Test]
        public void BoonOfChronum_UsesSharedVisitWithOneExtraChronum()
        {
            var service = CreateHistoricalTimewarpAnomalyService("BG34_Anomaly_801");
        ...
        [Test]
        public void MajorWaygate_ReplacesMinorWithMajorVisitsOnTurnsEightAndNine()
        {
            var service = CreateHistoricalTimewarpAnomalyService("BG34_Anomaly_802");
            var timewarp = service.State.Player.Tavern.Timewarp;
        ...
        [Test]
        public void TwistingHourglass_CarriesMinorChronumIntoMajorVisit()
        {
            var service = CreateHistoricalTimewarpAnomalyService("BG34_Anomaly_804");
            var timewarp = service.State.Player.Tavern.Timewarp;
        ...
        [Test]
        public void GoldthornPotions_UseSharedFiveSlotUiContractWithGoldenMinions()
        {
            var minor = CreateHistoricalTimewarpAnomalyService("BG34_Anomaly_800t");
            AdvanceToRoundKeepingTimewarp(minor, 6);
            AssertGoldthornVisit(minor, TimewarpKind.Minor);

            var major = CreateHistoricalTimewarpAnomalyService("BG34_Anomaly_800");
            AdvanceToRoundKeepingTimewarp(major, 6);
            major.Apply(new GameCommand(GameCommandType.ExitTimewarpedTavern));
            AdvanceToRoundKeepingTimewarp(major, 9);
            AssertGoldthornVisit(major, TimewarpKind.Major);
        }

        [Test]
        public void UnforeseenPortal_OpensExactlyOneDeterministicSharedVisit()
        {
            var service = CreateHistoricalTimewarpAnomalyService("BG34_Anomaly_809");
            var visits = 0;
            for (var round = 2; round <= 10; round += 1)
            {
                service.Apply(new GameCommand(GameCommandType.NextTurn));
                var timewarp = service.State.Player.Tavern.Timewarp;
                ...
                visits += 1;
                Assert.AreEqual("anomaly:unforeseen-portal", timewarp.PendingSource);
                Assert.AreEqual(5, timewarp.Offers.Count);
                service.Apply(new GameCommand(GameCommandType.ExitTimewarpedTavern));
            }
        ...
        private static MatchService CreateHistoricalTimewarpAnomalyService(string anomalyCardId)
        {
        ...
        private static void AdvanceToRoundKeepingTimewarp(MatchService service, int round)
        {
            while (service.State.Round < round)
            {
                service.Apply(new GameCommand(GameCommandType.NextTurn));
        ...
        private static void AssertGoldthornVisit(MatchService service, TimewarpKind kind)
        {
            var timewarp = service.State.Player.Tavern.Timewarp;
            Assert.IsTrue(timewarp.VisitOpen);
            Assert.AreEqual(kind, timewarp.PendingKind);
            Assert.AreEqual(5, timewarp.Offers.Count);
            Assert.IsTrue(timewarp.Offers.All(offer => offer.CardKind == CardKind.Minion && offer.Golden));
            Assert.IsTrue(service.GetTimewarpedOfferCards().All(card => card != null && card.Golden));
        }
