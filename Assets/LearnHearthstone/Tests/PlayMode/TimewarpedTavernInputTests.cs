    ...
    public sealed class TimewarpedTavernInputTests
    {
        ...
        public IEnumerator PlayMode_PJ04_TrinketFirstFullHandRapidClickShopRestoreAndCombatCompleteThroughRaycast()
        {
            ...
            try
            {
                ...
                yield return WaitForChild(rootObject.transform, "UnityTimewarpedTavernZone");
                ...
                var chronumBeforeFailure = tavern.Timewarp.Chronum;
                var countersBeforeFailure = CounterFingerprint(tavern);
                var logCountBeforeFailure = tavern.RecruitLog.Count;
                ...
                yield return WaitForToast(rootObject.transform, "UnityErrorToast", "手牌已满。");

                Assert.IsFalse(offer.Purchased);
                Assert.AreEqual(10, tavern.Hand.Count);
                Assert.AreEqual(chronumBeforeFailure, tavern.Timewarp.Chronum);
                Assert.AreEqual(countersBeforeFailure, CounterFingerprint(tavern));
                Assert.AreEqual(logCountBeforeFailure, tavern.RecruitLog.Count);

                var handAction = FindChild(rootObject.transform, "UnityCardAction-pj04-full-hand-0");
                ClickThroughRaycast(eventSystem, raycaster, handAction.GetComponent<RectTransform>());
                yield return WaitForHandCount(tavern, 9);
                yield return WaitForChild(rootObject.transform, actionName);

                var purchaseButton = FindChild(rootObject.transform, actionName).GetComponent<RectTransform>();
                var purchaseScreenPosition = RectTransformUtility.WorldToScreenPoint(
                    null,
                    purchaseButton.TransformPoint(purchaseButton.rect.center));
                ...
                ClickThroughRaycast(eventSystem, raycaster, purchaseButton);
                ClickAtScreenPosition(eventSystem, raycaster, purchaseScreenPosition);
        ...
        public IEnumerator PlayMode_TimewarpedOpenVisitBlocksNextTurnAndExitWorksThroughRaycast()
        {
            ...
            try
            {
                ...
                new UnityTavernTrainerView(rootObject.transform, service, new LocalAdvisorService(), () => { }).Build();
                yield return WaitForChild(rootObject.transform, "UnityQuickNextTurnButton");
                ...
                ClickThroughRaycast(eventSystem, raycaster, nextTurn.GetComponent<RectTransform>());
                yield return null;
                ...
                var timewarpedZone = FindChild(rootObject.transform, "UnityTimewarpedTavernZone");
                Assert.AreEqual("小型时空酒馆", FindChild(timewarpedZone, "UnityZoneTitle").GetComponent<Text>().text);
                Assert.AreEqual("时空资源 " + timewarp.Chronum, FindChild(timewarpedZone, "UnityZoneSubtitle").GetComponent<Text>().text);
                var offer = timewarp.Offers.First(item => item != null && item.SlotId == offerCard.InstanceId.Substring("timewarp-offer-".Length));
                var actionName = "UnityCardAction-" + offerCard.InstanceId;
                var handCountBefore = service.State.Player.Tavern.Hand.Count;
                timewarp.Chronum = 0;
                ...
                ClickThroughRaycast(
                    ...
                    raycaster,
                    FindChild(rootObject.transform, actionName).GetComponent<RectTransform>());
                yield return WaitForToast(rootObject.transform, "UnityErrorToast", "时空资源不足。");

                Assert.IsFalse(offer.Purchased);
                Assert.AreEqual(handCountBefore, service.State.Player.Tavern.Hand.Count);
                Assert.AreEqual(0, timewarp.Chronum);

                timewarp.Chronum = offer.Cost;
                yield return WaitForChild(rootObject.transform, actionName);
                ClickThroughRaycast(
                    eventSystem,
                    raycaster,
                ...
                Assert.IsTrue(offer.Purchased);
                Assert.AreEqual(0, timewarp.Chronum);
                Assert.AreEqual(handCountBefore + 1, service.State.Player.Tavern.Hand.Count);
        ...
        private static IEnumerator WaitForHandCount(TavernState tavern, int expected)
        {
            for (var frame = 0; frame < 60; frame += 1)
            {
                yield return null;
                Canvas.ForceUpdateCanvases();
                if (tavern.Hand.Count == expected)
                {
                    yield break;
                }
            }

            Assert.Fail("Timed out waiting for hand count " + expected + ".");
        }
        ...
        private static string CounterFingerprint(TavernState tavern)
        {
            return string.Join(
                "|",
                tavern.AdvancedMechanics.Counters.OrderBy(pair => pair.Key).Select(pair => pair.Key + ":" + pair.Value));
        }
