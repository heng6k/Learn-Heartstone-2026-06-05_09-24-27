...
> 状态：当前执行规范
> 生效日期：2026-07-30
> 适用范围：Learn Hearthstone 的内容数据、Unity 客户端、WebGL 外壳、Vercel 配置、重大功能版本与线上故障处理

## 1. 文档目的

本文规定“一个修改应该按什么类型提交、需要走哪些门禁、怎样上线以及怎样回滚”。具体构建和 Vercel 命令继续以 [WebGLUiChangeSyncAndDeploymentGuide.zh-CN.md](WebGLUiChangeSyncAndDeploymentGuide.zh-CN.md) 为准，本文不复制该操作手册。
...
- 所有线上变更至少经过 Preview；重大版本必须经过完整测试和线上 smoke 后才能 Promote。
- Production 必须 Promote 已验的同一 Preview deployment，不能重新构建一个“看起来相同”的 Production 包。
...
| Unity 小补丁 | 重新构建 WebGL，部署 Preview 并 Promote；旧 Production 在切换前一直可用 |
| Web 配置热修 | 复用已验 WebGL 输出组装新候选，先验 Preview，再 Promote |
...
| `CONTENT` 兼容内容补丁 | 文字、翻译、兼容 JSON、随从数值 | 否 | 新 `contentVersion` → 候选 → Preview → 内容/回退 smoke → Promote |
| `WEB` 网页或部署配置 | WebGL Template、Vercel headers、缓存、MIME、SPA 外壳 | 视情况而定 | 候选 → Preview → 浏览器完整 smoke → Promote |
| `CLIENT` Unity 客户端补丁 | C#、Prefab、场景、Unity UI、序列化资源 | 是 | 相关测试 → WebGL 构建 → 候选 → Preview → smoke → Promote |
| `FEATURE` 规则大更新或新功能 | 新机制、新模式、协议变化、跨模块行为 | 通常是 | 独立版本计划 → 全量门禁 → 新候选 → Preview → 完整验收 → Promote |
...
- Production 只能 Promote 已验 Preview。
- Promote 后立即验证正式域名和备用域名，并核对关键资源 ETag/长度。
...
4. 一次 Promote 原子切换到新 Production。
...
- Vercel 配置或安全头补丁通常复用已验 WebGL 输出，但必须重新组装候选并做浏览器 smoke。
...
- DNS、证书、代理或 Vercel Edge 故障先分层诊断，不通过重复构建 Unity 解决。
...
- 不在未授权时 push、tag、Promote 或删除历史 deployment。
...
| 改 Vercel headers、缓存或 SPA？ | 不一定重建 Unity，但必须组装新候选并验 Preview |
...
- [WebGLUiChangeSyncAndDeploymentGuide.zh-CN.md](WebGLUiChangeSyncAndDeploymentGuide.zh-CN.md)：具体构建、候选、Preview、Promote、Production 与回滚命令。
