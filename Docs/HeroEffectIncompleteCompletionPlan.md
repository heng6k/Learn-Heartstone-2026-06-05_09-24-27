...
| 状态 | 数量 | 处理方式 |
| --- | ---: | --- |
| `Planned` | 0 | 2026-07-04 修补后，Loh 与 Dinotamer Brann 已转 `Implemented`；当前没有注册表 `Planned` 英雄。 |
| `FrameworkFirst` | 25 | 已有部分代理或框架实现，必须先补共用机制再批量收口；Tavish、Tamsin、Onyxia、Bru'kan、Morchie、Scabbs、Tess、Galewing、Yogg 已在 2026-07-04 转 `Implemented`。 |
| `Deferred` | 0 | 当前注册表不再保留 `Deferred` 英雄；大系统缺口以 `FrameworkFirst` 和文档边界记录。 |
...
| ---: | --- | --- | --- | --- | --- | --- |
| 1 | Galewing / Flight Trainer | `Implemented` | 航线 pending choice、延迟回合奖励、路线历史 | 无 | 已完成航线状态、倒计时、不能连续重复选择、Westfall/Ironforge/Eastern Plaguelands 官方奖励和 Flight Trainer 双结算 | 英雄选择路线、倒计时发奖、连续路线过滤、宝宝双奖励测试已补 |
...
- Galewing 三条航线已按当前确认奖励表完成：Westfall 1 回合后随机 1 费 Tavern spell；Ironforge 2 回合后 2 金币；Eastern Plaguelands 3 回合后发现当前酒馆等级随从。
...
| 3 | Yogg-Saron / Acolyte of Yogg-Saron | `Implemented` | 随机 Tavern spell 执行入口、Wheel of Yogg 结果表 | 官方 public API 未展开 Wheel 子结果；当前采用用户确认的项目五格表 | Turn 3 起回合开始从合法 Tavern spell 池随机施放；宝宝回合开始使用五格 Wheel：开炮、Darkmoon Prize、4 个 Tavern spell、身材转移、吞噬刷新 | 已补解锁时点、随机法术候选、共享法术执行入口、五格轮盘状态测试 |

待确认：

- Darkmoon Prize 各级奖品效果已补齐，生成奖品不再保留 `darkmoon_prize_proxy` 标记；Tickatus 已可作为完整实现验收。
- Wheel of Yogg 已按用户确认的项目五格表转 `Implemented`；官方 public API 未展开子结果，来源边界保留在计划和 registry note 中。
...
| P3.5 | Genn, Worgen King | 已实现：非 Cosmic Duality 局中，Turn 4 发现两个英雄技能替换 `King of Duality`，一个作为主技能、一个作为额外技能 | Cosmic Duality 局禁用 Genn；Turn 4 多选、两个持久技能、回合持久化和 focused tests |
...
| Genn, Worgen King | `Implemented` | Turn 4 replacement Discover 已接入；Cosmic Duality 局禁用；两个持久技能槽通过主技能 + `ExtraHeroPowerCardIds` 落地 | `King of Duality` 被替换后不再可用；两个新技能都能被显示和使用 |
...
| --- | --- | --- | --- | --- | --- |
| `Implemented` | Mister Clocksworth | Double Time | `BG34_HERO_002p` | 允许 | 已完成：两张合金和 Tavern Coin 替换已接上 |
...
| `Implemented` | Deathwing | ALL Will Burn! | `TB_BaconShop_HP_061` | 允许 | 已完成；对手永久写回要求已按用户决策删除 |
| `Implemented` | Galewing | Dungar's Gryphon | `BG20_HERO_283p` | 允许 | 已完成：三航线奖励表和 Flight Trainer 双触发 |
...
| `Implemented` | Scabbs Cutterbutter | I Spy | `BG21_HERO_010p` | 允许 | 已判定：按单人模拟对手代理完成 |
...
| `Implemented` | Tess Greymane | Bob's Burgles | `TB_BaconShop_HP_077` | 允许 | 已判定：按上次模拟对手快照完成 |
...
| `Implemented` | Yogg-Saron, Hope's End | Puzzle Box | `TB_BaconShop_HP_039t` | 允许 | 已完成：Puzzle Box 和用户确认五格 Wheel |
| `Implemented` | Lady Vashj | Relics of the Deep | `BG23_HERO_304p` | 允许 | 已完成：不高于当前酒馆等级的临时 Spellcraft |
| `Implemented` | Lord Barov | Friendly Wager | `TB_BaconShop_HP_081` | 允许 | 已完成：单人预测选择与结算代理 |
| `Implemented` | Master Nguyen | Power of the Storm | `BG20_HERO_202p` | 已完成；不再作为未完成候选待判定 | 已完成 |
| `Implemented` | Murloc Holmes | Detective for Hire | `BG23_HERO_303p2` | 允许 | 已完成：单人对手快照竞猜代理 |
| `Implemented` | Queen Azshara | Azshara's Ambition | `BG22_HERO_007p` | 允许 | 已完成：30 攻阈值和 Imperial Defender Spellcraft 复制 |
| `Implemented` | The Rat King | A Tale of Kings | `TB_BaconShop_HP_041` | 允许 | 已完成：回合开始种族轮换和当前类型 Discover |
...
| --- | --- | --- | --- | --- | --- |
| `InitialOnly` | `Implemented` | Genn, Worgen King | King of Duality | `BG35_HERO_001p` | Cosmic Duality 下禁用；非 Cosmic Duality Turn 4 双技能替换 |
...
| `InitialOnly` | `Implemented` | Dinotamer Brann | Battle Brand | `TB_BaconShop_HP_048` | 不进普通候选；英雄和 Brann's Epic Egg 已完成 |
| `InitialOnly` | `Implemented` | Loh, the Living Legend | Heroic Inspiration | `BG33_HERO_001p_ALT` | 不进普通候选；英雄和 Stoneshell Guardian 已完成 |
| `Disabled` | `Implemented` | Artanis | Warp Gate | `BG31_HERO_802p` | 不进普通候选；Protoss 二选一和买 14 张牌奖励已完成 |
| `Disabled` | `FrameworkFirst` | Buttons | Growing Collection | `BG32_HERO_002p` | 不进普通候选 |
| `Disabled` | `Implemented` | Jim Raynor | Lift Off | `BG31_HERO_801p` | 不进普通候选；Terran/Battlecruiser 主路径已完成 |
| `Disabled` | `Implemented` | Kerrigan, Queen of Blades | Spawning Pool | `BG31_HERO_811p` | 不进普通候选；Zerg morph 主路径已完成 |
| `Disabled` | `FrameworkFirst` | Marin the Manager | Fantastic Treasure | `BG30_HERO_304p` | 不进普通候选 |
| `Disabled` | `Implemented` | Morchie | Warped Conflux | `BG34_HERO_004p` | 不进普通候选；已按 Turn 5 Minor Timewarped Tavern 完成标准转正 |
| `Disabled` | `Implemented` | Murozond, Unbounded | Alternate Timeline | `BG34_HERO_000p` | 不进普通候选；英雄本体 Turn 8 Major Timewarped Tavern 已完成 |
...
| 4 | Scabbs / Warden Thelwater | `Implemented` | 下一对手排程、对手战队快照、Buddy 映射 | 当前下一对手采用单人模拟对手代理 | 已按用户决策转正；真实大厅排程不是当前单人训练器完成阻塞项 | 代理路径、无对手快照、Buddy 获取测试 |
| 5 | Tess / Hunter of Old | `Implemented` | 上一对手战队快照、上一对手 Buddy | 单人训练器使用上一场/模拟对手快照 | 已按用户决策转正；真实大厅接入后可替换来源但不阻塞当前完成状态 | Bob's Burgles 刷新、宝宝获取、无历史回退测试 |
...
| 战斗开始英雄触发排序 | Illidan、Al'Akir；Bru'kan、Deathwing 已用 focused 路径完成，保留为回归样例 |
...
4. 亡语 payload 和召唤位置：收口 N'Zoth、Sneed、Teron、The Jailer、Al'Akir；Dinotamer Brann 已用 focused Deathrattle payload 转 `Implemented`，后续只作为通用 payload 回归样例。
...
- `Planned`：无。
- `FrameworkFirst`：Al'Akir、Illidan、N'Zoth、Teron、Rafaam、Rokara、Sylvanas、Sneed、The Jailer、Greybough、Ini Stormcoil、Ozumat、Aranna、Lord Jaraxxus、Vol'jin、Shudderwock。Tavish、Tamsin、Onyxia、Bru'kan、Deathwing、Loh、Dinotamer Brann 已在 2026-07-04 转 `Implemented`。
...
| --- | --- | --- | --- | --- | --- | --- |
| Spellcraft / Naga | Lady Vashj / Coilfang Elite；Queen Azshara / Imperial Defender | `Implemented` | Spellcraft 临时法术、回合结束清理、Naga Conquest、友方法术复制 | 已接入英雄与宝宝闭环 | 后续只补更细的官方 Spellcraft 数据差异（如有） | 临时牌清理、复制候选、阈值解锁、每回合一次测试 |
...
| TripleEngine | Mister Clocksworth | `Implemented` | 两张即可合金、三连奖励替换为 Tavern Coin | 已复用可配置 `requiredCopies` 路径 | 后续只维护普通三连/False Idols 回归测试 | 两张合金、普通三连不受影响、奖励替换测试 |
...
| StarCraft | Jim Raynor / Tychus；Artanis / Probius；Kerrigan / Broken Horn | `Implemented` | Terran/Battlecruiser、Protoss 奖励、Zerg morph | 主路径已接官方子卡/Discover/战斗钩子 | 后续只做 UI/资产/官方数值微调，不再阻塞英雄完成度 | 各种族专属池、升级链、延迟奖励、变形限制测试 |
| Timewarped / Timeline | Murozond | `Implemented` | Major Timewarp 已打开；时间线和对手历史扩展属于 Timewarped Tavern 牌/数据底座 | Murozond 英雄本体已完成；previous-warband 不挂在英雄完成度上 | 保留已完成开放时点；若未来具体 Timewarp 牌需要时间线快照，再放入 Timewarped Tavern 数据底座 | Turn 8 开门回归、Timewarp 牌效果测试 |
...
3. Secret、Trinket、StarCraft 这类大机制，如果短期不能一次做全，哪些允许先以 proxy 状态上线。Darkmoon Prize 与 Yogg Wheel 本轮已按项目标准完成。
4. 官方触发顺序缺失时，是否采用项目内部统一顺序，还是等你提供具体规则。

## 下一步执行建议

1. P0-P4 本轮已收口：Galewing、Yogg、Loh、Dinotamer Brann、Timewarped fixed legacy offer 均已完成并同步 registry。
2. 下一批不再从 Planned 英雄开工，而是从剩余 24 个 `FrameworkFirst` 中按公共底座推进。
3. 优先顺序建议：战斗事件/亡语 payload 泛化 -> Trinket -> StarCraft Terran -> StarCraft Zerg -> Protoss -> Secret / Custom Undead。
4. 继续保留 focused 回归，不把已完成英雄状态回退成代理。
