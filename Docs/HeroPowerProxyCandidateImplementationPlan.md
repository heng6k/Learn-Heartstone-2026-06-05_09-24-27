...
| --- | --- | --- | --- | --- | --- | --- |
| Implemented | 时钟先生 | Double Time / 双倍速 | `BG34_HERO_002p` | 官方 API：2 复制成金，金色随从不给三连奖励、改给酒馆币。 | 开放，无代理标签 | 已完成二复制 TripleEngine 与 Tavern Coin 替换。 |
...
| Implemented | 死亡之翼 | ALL Will Burn! / 万物尽焚 | `TB_BaconShop_HP_061` | 官方 API：战斗开始所有随从永久 +2 攻击。 | 开放，无代理标签 | 已完成玩家侧永久攻击、战斗副本双方 +2 攻击和 Sinestra 生命转换；对手永久写回按单人训练器边界删除。 |
| Implemented | 风翼 | Dungar's Gryphon / 杜加尔的狮鹫 | `BG20_HERO_283p` | 官方 API：选择航线，完成获得奖励。 | 开放，无代理标签 | 已接 Westfall/Ironforge/Eastern Plaguelands 奖励表和 Flight Trainer 双触发。 |
...
| Implemented | 斯卡布斯 | I Spy / 间谍探查 | `BG21_HERO_010p` | 官方 API：发现下个对手战队随从原始复制。 | 开放，无代理标签 | 已按单人模拟对手代理完成，真实排程不阻塞当前训练器。 |
...
| Implemented | 苔丝 | Bob's Burgles / 鲍勃的豪夺 | `TB_BaconShop_HP_077` | 官方 API：刷新酒馆为上个对手战队原始复制。 | 开放，无代理标签 | 已按上次模拟对手快照完成，真实多玩家历史不阻塞当前训练器。 |
...
| Implemented | 尤格萨隆 | Puzzle Box / 迷之匣 | `TB_BaconShop_HP_039t` | 官方站点 API 未返回；HearthstoneJSON：第 3 回合后回合开始施放随机酒馆法术。 | 开放，无代理标签 | 英雄技能本体已可玩；Acolyte 使用用户确认的五格 Wheel 表，当前按项目完成标准转正。 |
| Implemented | 瓦丝琪女士 | Relics of the Deep / 深海遗物 | `BG23_HERO_304p` | 官方 API：每回合开始随机获得 Spellcraft 法术。 | 开放，无代理标签 | 已完成不高于当前酒馆等级的临时 Spellcraft 和 Coilfang Elite 复制。 |
| Implemented | 巴罗夫领主 | Friendly Wager / 友好投注 | `TB_BaconShop_HP_081` | 官方 API：猜下场战斗胜者，猜中得 3 张酒馆币。 | 开放，无代理标签 | 已完成单人预测选择、结算和 Barov's Apprentice Coin 触发。 |
...
| Implemented | 鱼人福尔摩斯 | Detective for Hire / 特邀侦探 | `BG23_HERO_303p2` | 官方 API：看 2 个随从，猜中来自下个对手上一场战斗的随从得酒馆币。 | 开放，无代理标签 | 已完成单人对手快照竞猜和 Watfin 正确后复制。 |
| Implemented | 艾萨拉女王 | Azshara's Ambition / 艾萨拉的野心 | `BG22_HERO_007p` | 官方 API：战队 30 攻后开启纳迦远征。 | 开放，无代理标签 | 已完成 30 攻 Naga Conquest 和 Imperial Defender 每回合一次 Spellcraft 复制。 |
| Implemented | 鼠王 | A Tale of Kings / 鼠王的故事 | `TB_BaconShop_HP_041` | 官方 API：发现特定类型随从，每回合切换类型。 | 开放，无代理标签 | 已按用户语义完成：回合开始随机切换类型，发现使用当时当前池；Buddy 只在特殊规则已入池时自然出现。 |
...
3. 直接过滤组：Bigglesworth 和所有仍未完成的 `Planned` / `Deferred` / `Unregistered` 默认不进候选，除非你逐项允许代理。
4. 高价值补全组：`Yogg Puzzle Box` 与 `Deathwing` 候选标签已清理；后续优先补 `Rokara/Al'Akir` 这类已有战斗入口的 focused 回归，再处理剩余战斗事件依赖技能。
5. 大系统/过滤组：Secret 等仍未完成大系统单独开批；Bigglesworth 暂时直接过滤。
...
- Directly filtered from replacement/discover pools: Bigglesworth and all non-offerable `Planned`/`Deferred`/`Unregistered` Hero Powers. Double Time, Barov, Holmes, Vashj, Azshara, Rat King, Galewing, Yogg, Loh, and Dinotamer Brann are now implemented and may enter candidate pools when otherwise eligible.
