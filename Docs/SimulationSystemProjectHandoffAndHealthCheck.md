...
This pass covers seven practical steps:
...
| Quest and Trinket systems | `QuestSystemTests` 58/58, `QuestTrinketInteractionTests` 10/10, `TrinketSystemTests` 220/220 | Passed | Standalone Quest/Trinket behavior and cross-system interactions are healthy. The focused interaction tests now lock same-combat reward convergence, combat reward visibility before next recruit refresh, Timewarped summon rewards feeding Trinket summon counters, and repeated deathrattle-created summons advancing summon counters once per real summon while opponent repeated summons stay isolated. |
